#!/bin/bash
# Webhook Self-Update Script
# This script updates the GitHub App token generation on the bastion host
# It's designed to be run by the webhook deployment script itself

set -e

echo "Updating GitHub App token generation script..."

# Update the GitHub App token generation script with correct installation ID
sudo tee /opt/webhook/github-app/get-token.sh << 'EOL'
#!/bin/bash
...
echo "GitHub App token generation script updated successfully!"

# Test the new script
echo "Testing GitHub App authentication..."
if sudo /opt/webhook/github-app/get-token.sh > /dev/null 2>&1; then
...
echo "Webhook self-update completed successfully!"